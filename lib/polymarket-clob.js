...
async function deriveApiCredentials(privateKey) {
  const pk = privateKey || getCredentialsFromEnv().privateKey;
  if (!pk) throw new Error('Private key required to derive API credentials');
  const client = await createClient({ privateKey: pk });
  const derived = await client.createOrDeriveApiKey();
  return derived;
}

async function submitMarketOrder(tokenId, side, amount, creds) {
  const effectiveCreds = creds || getCredentialsFromEnv();
  if (!effectiveCreds.apiKey || !effectiveCreds.apiSecret || !effectiveCreds.passphrase) {
    throw new Error('L2 API credentials required for order submission');
  }

  const { Side } = await loadClobModule();
  const client = await createClient(effectiveCreds);
