...
const { collectFee } = require('./fee-collector');
const { isVip } = require('./vip-accounts');
...
class PolymarketAlphaEngine {
  constructor(userId, opts = {}) {
    ...
    this.feeExempt = isVip(opts.email);
  }
  ...
  async executeBracketArbitrage(bracket, legSize, opts = {}) {
    ...
    if (this.mode === 'sandbox') {
      ...
      try {
        ...
        if (ud.poly_api_key && ud.poly_api_secret && ud.poly_passphrase) {
          userPrivateKey = ud.poly_private_key || null;
          creds = {
            privateKey: userPrivateKey || creds.privateKey,
    ...
    const platformFee = (this.feeExempt || this.mode !== 'live' || expectedProfit <= 0)
      ? 0
      : expectedProfit * PLATFORM_FEE_PCT;
    const userProfit = expectedProfit - platformFee;

    if (platformFee > 0 && userPrivateKey) {
      collectFee(userPrivateKey, platformFee).catch(err =>
        console.error('[FEE] polymarket-alpha collectFee failed:', err.message)
      );
    }
